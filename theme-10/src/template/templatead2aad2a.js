...
var wedding_gift_form = function(e) {
    ...
    var onSuccess = function(res) {
        // Persist gift state for templates that show a confirmation card
        localStorage.setItem('theme10_wedding_gift_sent', '1');

        var $giftForm = $('.wedding-gift-form');
        var $giftConfirm = $('#weddingGiftConfirmCard');
        if ($giftForm.length && $giftConfirm.length) {
            $giftForm.hide();
            $giftConfirm.show();
        }
        ...
        if (!res.wedding_gift_message && !($giftForm.length && $giftConfirm.length)) {
    ...
    if (typeof window.BANK_OPTIONS !== 'undefined' && window.BANK_OPTIONS) {
        ...
        if ($(el).length) {
            ...
            var options = selectize_options({
                            ...
                            valueField: 'id',
                            labelField: 'title',
                            searchField: ['account_name', 'account_number'],
                            options: (window.BANK_OPTIONS ? window.BANK_OPTIONS : []),
                            render: {
                                item: function(item, escape) {
                                    var title = item.bank_name;
                                    return '<div>' + (title ? '<p class="select-bank__title">' + escape(title) + '</p>' : '') + '</div>';
                                },
                                option: function(item, escape) {
                                    var title = item.account_name;
                                    var credential = item.account_number;                        
...
var AudioManager = {
    ...
    state: {
        ...
        retryAttempts: 0,
        maxRetries: 3,
        loopStartTime: window.CROPPED_SONG.start,  // Mulai dari detik ke-5
        loopEndTime: window.CROPPED_SONG.end    // Loop kembali saat mencapai detik ke-15
    },
    ...
    window.updateQuestionWrappers = function() {  
        try {
            ...
            $(".rsvpPlus-body").each(function(stepIndex) {
                ...
                const guestName = $body.data('guest');
